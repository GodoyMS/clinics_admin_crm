                                ...
                                <div class="px-6 py-6 lg:px-8">
                                  ...
                                  <form class="space-y-6" action="" method="post" >
                                        ...
                                        <div>
                                          ...
                                          <textarea id="linkWhatsappMessage<?=$fetch_evento['nombreEvento'].$fetch_evento['id'];?>" value=""  onkeyup="createUrl('result<?=$fetch_evento['nombreEvento'].$fetch_evento['id'];?>','linkWhatsappMessage<?=$fetch_evento['nombreEvento'].$fetch_evento['id'];?>','<?=$fetch_evento['nombreEvento'];?>' )"  rows="10" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500      " >Hola <?=$fetch_evento['nombreEvento'];?>, tu cita ha sido programada para <?=$year.'-'.$monthString.'-'.$day;?> a las <?=$exactTime;?> Horas. Para confirmar, postergar(Horarios disponibles) o cancelar tu cita, ingresa al siguiente enlace con tu dni: <?=$clientURLRegistro;?>. Tu token digital es: <?=$fetchedToken;?>. Si ya estas registrado(a), ingresa aqui: <?=$clientURL;?>. ¡Que tengas un buen dia! </textarea>
                  ...
                  <div id="dropdownDotsHorizontal<?=$fetch_evento['id'];?>" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-44  ">
                      ...
                      <input hidden name="idEvento" type="text" value="<?=$fetch_evento['id'];?>" >
                        ...
                        <button  name="submitCancelarCita" type="submit" class=" w-full flex gap-4 items-center justify-start block px-2 py-2 bg-red-500 text-white hover:bg-red-600   width-full">
